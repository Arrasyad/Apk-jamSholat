...
class MainActivity : AppCompatActivity() {
    ...
    // Menggunakan Looper.getMainLooper() adalah praktik yang lebih aman
    ...
    private val requestNotificationPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted: Boolean ->
            if (isGranted) {
                ...
                Toast.makeText(this, "Tanpa izin, notifikasi adzan tidak akan muncul.", Toast.LENGTH_LONG).show()
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        requestNotificationPermission() // Meminta izin notifikasi saat startup
        updateCurrentTime()
        showRandomQuote()
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Channel untuk notifikasi suara adzan"
            }
    ...
    // Fungsi baru untuk meminta izin notifikasi di Android 13+
